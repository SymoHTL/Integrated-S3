...
- Status update:
  ...
  - S3 XML response writers now emit the canonical `http://s3.amazonaws.com/doc/2006-03-01/` namespace across versioning, CORS, error, copy, list, multipart, delete, and tagging XML payloads, and namespace-aware protocol/HTTP tests lock in the response shape
...
- Remaining scope:
  - next: continue versioning/tagging/delete-marker parity work for the remaining advanced edge cases now that the current conditional, checksum-header validation, raw-query SigV4, and canonical XML namespace gaps are covered
