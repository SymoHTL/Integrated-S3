...
| SigV4 header authentication | implemented | n/a | implemented | yes | authorization-header validation is covered in HTTP and AWS SDK tests, including current space-containing key paths that arrive pre-escaped on the supported surface |
...
| checksums | partially implemented | emulated | partially implemented | yes | disk now computes and persists checksums for the current put/copy/multipart flows, the native S3 provider now forwards checksum algorithms plus explicit checksum headers and echoes checksum data on native put/copy/multipart paths where the upstream SDK exposes it cleanly, and the current HTTP surface now accepts `CopyObject` and `UploadPartCopy` checksum request headers, emits the supported checksum fields plus single-algorithm `x-amz-checksum-algorithm` on successful checksum-declared `PutObject` and `CopyObject` responses plus requested checksum fields on `UploadPartCopy` responses, and enforces direct-write-style declared-checksum consistency plus trailer-backed `aws-chunked` validation for current `PUT` / `UploadPart` writes, including required `x-amz-trailer-signature` verification for `STREAMING-AWS4-HMAC-SHA256-PAYLOAD-TRAILER`, canonicalized trailer-header hashing so mixed-case signed checksum trailers now validate correctly, success-path plus `BadDigest` mismatch parity for unsigned trailer-backed SHA256, SHA1, CRC32, and CRC32C checksum trailers and signed trailer-backed SHA1, CRC32, and CRC32C checksum trailers on both raw HTTP and SigV4 header-auth `PUT Object` / `UploadPart` flows, and focused AWS SDK compatibility coverage proving signed-trailer SHA1 `PutObject` / `UploadPart` interoperability over the current HTTP harness; broader checksum-override, multipart-finalization/read-mode, broader AWS SDK signed-trailer algorithm coverage beyond SHA1 still needs CRT-enabled SDK coverage, and official AWS SDK unsigned-trailer compatibility remains pending |
| ACL / policy behavior | implemented for the current slice | emulated | implemented for the current slice | yes | the platform now layers a narrow ACL/policy compatibility model over `ClaimsPrincipal` authorization: bucket/object `private` and `public-read` plus `bucket-owner-full-control` normalization, `GET` / `PUT ?acl`, `GET` / `PUT` / `DELETE ?policy`, anonymous `ListObjects` / `HeadBucket` / `GetObject` / `HeadObject` fallback for stored public grants, and explicit `NotImplemented` errors for unsupported grants, conditions, version-specific ACLs, and multipart-initiation ACL headers |
...
- ACL/policy compatibility is now implemented for a narrow public-access slice: current bucket/object canned ACLs (`private`, `public-read`, `bucket-owner-full-control` normalization), S3-compatible bucket/object `GET` / `PUT ?acl`, limited anonymous bucket policies (`s3:ListBucket`, `s3:GetObject`, `s3:GetObjectVersion` with bucket/object ARN resources), and authorization fallback for anonymous/public `ListObjects`, `HeadBucket`, `GetObject`, and `HeadObject`
- unsupported ACL grant headers, version-specific ACLs, multipart-initiation ACL headers, richer bucket policy principals/actions/resources/conditions, object lock / retention / legal hold, and encryption-related follow-ons still remain outside the current implemented slice and now fail explicitly instead of being ignored
...
- Verification status (March 2026):
  - `dotnet build src\IntegratedS3\IntegratedS3.slnx --nologo` passed in the current Track B/C/H worktree; the endpoint-map cleanup now leaves no `WebUi` / `IntegratedS3.AspNetCore` IL warnings in this build, although the test project still emits four existing CS8602 nullable warnings in `DiskStorageServiceTests.cs` / `S3StorageServiceTests.cs`.
  - Focused reruns for the newly touched areas also passed: `IntegratedS3CoreOrchestrationTests` (`57/57`), `IntegratedS3ClientTransferTests` (`57/57`), `S3CompatibleEndpointConformanceTests` (`19/19`), and `HotPathBenchmarkReportTests` (`3/3`).
  - `dotnet test src\IntegratedS3\IntegratedS3.slnx --nologo` passed (`739/739`).
  - `eng\Invoke-AotPublishValidation.ps1` passed with its forced `dotnet publish -t:Rebuild` validation and observed `0` IL2026/IL3050 warnings, so `WebUiApplication.ConfigurePipeline(...)` now publishes with a zero-warning endpoint-map posture.
  - `dotnet run --configuration Release --project src\IntegratedS3\IntegratedS3.Benchmarks\IntegratedS3.Benchmarks.csproj -- --list-scenarios` confirmed the checked-in 19-scenario benchmark catalog, including the new loopback HTTP and AWS SDK metadata-lookup comparison scenarios.
  - `INTEGRATEDS3_S3COMPAT_*` was unset in this validation environment, so the opt-in local S3-compatible historical copy/direct `PUT`/version-marker/multipart checksum-plus-SSE scenarios were compile/early-return validated here and still need configured real-endpoint reruns.
- Remaining scope:
...
Status note (March 2026): current Track H validation covers checklist items 5, 6, 8, 9, 10, and 11 at the command/test level; item 5 now publishes the current `WebUi` host without IL2026/IL3050 warnings, item 6 now includes SSE-C customer-provided-key header parsing, validation (AES256-only algorithm, completeness, mutual exclusivity with managed SSE), provider delegation across all applicable S3 operations, disk-provider rejection, and comprehensive SSE-C response header parity across PutObject/CopyObject/InitiateMultipartUpload/UploadPart/UploadPartCopy/CompleteMultipartUpload, and item 9 now has a checked-in 19-scenario hot-path baseline that now includes repo-local loopback HTTP plus AWS SDK metadata-lookup comparison coverage alongside the earlier body-flow client-comparison slice, with native-S3 and broader client-comparison expansion still open. Dedicated rclone S3 compatibility coverage now exercises 54 focused scenarios covering listing (V1/V2 with delimiter, prefix, pagination, encoding-type=url, fetch-owner, start-after, versioning), CRUD (Content-MD5 validation, metadata mtime round-trip, range/conditional GET, batch delete, copy-to-self with REPLACE, cross-bucket copy, directory markers, Content-Type handling), multipart (full lifecycle, metadata, list uploads/parts, pagination, abort, ETag format), encoding (spaces, plus, Unicode, URL-special, directory-like paths, copy-source encoding), and error handling (NoSuchKey, NoSuchBucket, NoSuchUpload, invalid part order, request-id, error Content-Type). Two endpoint bugs fixed during rclone hardening: Content-MD5 was silently ignored (now flows through checksum validation to return BadDigest), and CompleteMultipartUpload returned InvalidRequest instead of NoSuchUpload for non-existent upload IDs. Current test baseline: 855 tests, 0 publish warnings.
...
Track A's contract slice, Track B's native S3 copy/multipart/checksum/delegated-read slice, Track B's version-targeted same-key self-copy plus local historical read/presign conformance slice, Track C's direct/delegated presign-client slice plus safer resume-download checksum/range hardening, Track E's broader signed-trailer plus versioning/delete-marker hardening slices, Track G's bucket default encryption auto-application/write-time parity slice, and the latest Track H backlog-coalescing / benchmark-expansion plus Track B/C follow-on hardening batch are now in place. The opt-in local S3-compatible conformance harness now also exercises historical `CopyObject` and `UploadPartCopy` after a current delete marker with requested `SHA256` plus `AES256` SSE, plus direct presigned `PUT` after a delete marker, while the first-party transfer helper now re-presigns restart-from-zero follow-on `GET` requests whenever resume recovery abandons the original ranged request. The hot-path benchmark suite now covers 19 repo-local scenarios, including loopback HTTP and AWS SDK metadata-lookup comparison paths on top of the earlier disk/loopback/body-flow client-comparison coverage, and Track H orchestration tests now lock in same-key backlog coalescing without saturating superseded repair work. The next realistic bigger-stack move is to continue Track H broader repair/reconciliation plus benchmark expansion beyond the current 19-scenario disk/loopback/AWS-SDK baseline in parallel with Track B configured real-endpoint reruns of the newer checksum/SSE historical copy/multipart cases, Track F/G retention-legal-hold/ACL-policy/health-admin follow-ons, and Track C live-endpoint/composite/discovery-helper ergonomics, while keeping the zero-warning rebuild-backed publish validation pinned in CI. Broader SDK trailer algorithms beyond SHA1 still hinge on `AWSSDK.Extensions.CrtIntegration`, and unsigned trailer mode still needs `DisablePayloadSigning=true` over HTTPS.
...
- the new provider-mode and object-location contracts already reduce abstraction churn, so remaining Track B / Track C work is mostly hardening, data-integrity polish, and optional backend-direct follow-on behavior rather than another foundational redesign
...
1. execute Track H to expand backlog-saturation/topology-change fault injection, broader repair/reconciliation coverage, benchmark expansion beyond the current 19-scenario disk/loopback/AWS-SDK baseline, and zero-warning publish/AOT verification around future host/integration changes
2. execute Track B in parallel to rerun and widen real-endpoint validation of the landed native S3 copy/multipart/checksum/SSE/delegated-read slice against MinIO/LocalStack-style endpoints, especially the newer direct historical checksum/SSE copy, direct `PUT`, multipart copy, and version-marker cases, and decide whether backend-direct presign should complement the current resolver path
3. continue Track F in parallel and use the remaining Track G budget for retention/legal-hold, ACL/policy parity follow-on work, richer multipart/SSE surfacing, and health/admin visibility now that endpoint toggles, repair-backlog visibility, descriptor follow-ons, CORS `Vary` hardening, and the initial public-access ACL/policy slice are in place
4. continue Track C in parallel with broader post-resume client ergonomics now that direct/delegated read paths, typed transfer helpers, resume-aware file downloads, range-start validation, checksum-rollback protection, and restart-from-zero re-presigning are already shipped
...
6. reuse the new native-S3-provider-backed AWS SDK loopback harness opportunistically for future encryption/client parity that exposes observable SDK metadata, rather than treating harness creation itself as the next dedicated slice

## Suggested First Parallel Batch

Given the current implementation state, the first parallel batch should be:

1. Track H — extend fault-injection coverage, broaden benchmarks beyond the current 19-scenario disk/loopback/AWS-SDK baseline, and keep publish/AOT verification at the new zero-warning posture
2. Track B — rerun and broaden local S3-compatible endpoint validation for historical read/copy/presign semantics, multipart/copy, checksum, SSE, direct `PUT`, and version-marker pagination behavior against configured real endpoints
3. Track F / Track G follow-on — keep pushing retention/legal-hold, ACL/policy parity, health/admin visibility, and richer multipart/read-side SSE surfacing while the current endpoint/provider slices are stable
4. Track C follow-up — validate broader composite-checksum, live-endpoint, and discovery-helper ergonomics on top of the current proxy/direct/delegated presign surface and the landed resume range/rollback integrity guards plus restart-from-zero re-presigning
