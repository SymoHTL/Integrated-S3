...
- Remaining scope:
  - next: continue versioning/tagging/delete-marker parity work for the remaining advanced edge cases now that the current conditional, checksum-header validation, raw-query SigV4, and multipart listing gaps are covered
  ...
  - continue tightening deeper multipart-listing/client-compat edge semantics, including whether optional `ListParts` owner/initiator fields should be implemented next or remain explicitly unsupported for now
