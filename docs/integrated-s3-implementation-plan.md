...
- `StoragePresignRequest` / `StorageDirectObjectAccessRequest` now also carry optional checksum algorithm + checksum values for checksum-aware PUT presigns, and the first-party HTTP presign strategy signs and replays the corresponding `x-amz-sdk-checksum-algorithm` / `x-amz-checksum-*` headers for proxy-mode grants
...
- checksum-aware upload overloads now give callers a supported high-level path for larger file transfers without full buffering: the client computes the requested checksum from the current stream/file position, requests a checksum-aware presign grant, replays the signed checksum headers during upload, and validates full-object checksum headers on download and resume flows when the server returns non-composite values; checksum-aware uploads intentionally require seekable streams (or `UploadFileAsync`), and multipart/composite download checksum headers are intentionally surfaced but not client-validated yet
